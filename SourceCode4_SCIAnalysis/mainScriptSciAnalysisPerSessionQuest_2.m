%% paths to data and save paths from job submission on command line

% dir_path, session, save_path, micronsPerPixel
%micronsPerPixel = 2.5; %2.5 microns (inscopix), 1 (v3), 1.85 (v4)

% restrict MATLAB to nodes reserved in submission script 
maxNumCompThreads(str2num(getenv('SLURM_NPROCS')));
addpath(genpath('spatial_clustering'));

%% analysis parameters - shared across miniscope configurations 
