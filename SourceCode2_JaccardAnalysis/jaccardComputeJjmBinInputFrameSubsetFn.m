...
function [outputFileArray] = jaccardComputeJjmBinInputFrameSubsetFn(session, frameFileInput, ...
    ...
    % flag to analyze all frames or subset
     
    if strcmp(frameFileInput, 'all_frames')

        save_path = strcat(dir_path,frameSubsetDir, '\', session, '_'); 
        within_frames_to_analyze = 'all'; 
        
        disp('analyzing all frames')
        disp(save_path)

    else 
    
        save_path = strcat(frameSubsetDir, 'analysisOutput\' , erase(frameFileInput, '.csv') , '_');
